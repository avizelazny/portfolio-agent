...
You also analyze mutual funds (קרנות נאמנות) — both tracking funds (TTF/קרנות מחקות) and money-market funds (קרנות כספיות). You understand NAV trends, fund-level exposure, and how fund performance relates to underlying indices.
...
def _build_context(portfolio, signals, macro, index_perf, news_chunks, run_type, performance_text=None, funds_text=None):
    holdings_text = "\n".join([
    ...
    funds_section = funds_text or "נתוני קרנות לא זמינים להיום."
...
<task>
Generate a {run_type} investment report. Include analysis of both individual stocks AND mutual funds.
For mutual funds: note NAV trends, flag any unusual daily changes, and consider rebalancing between money-market and tracking funds based on macro context.
...
Give 8-12 recommendations. Cover: individual stocks (from signals) AND mutual fund moves (TTF/כספית rebalancing).
For fund recommendations use fund ID as ticker (e.g. "5136544").
...
class PortfolioAgent:
    ...
    def generate_report(self, portfolio, signals, macro, index_perf, news_chunks, run_type="morning", performance_text=None, funds_text=None):
        start = time.monotonic()
        context = _build_context(portfolio, signals, macro, index_perf, news_chunks, run_type, performance_text, funds_text)
